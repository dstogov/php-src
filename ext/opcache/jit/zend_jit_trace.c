...
static zend_jit_reg_var* zend_jit_trace_allocate_registers(zend_jit_trace_rec *trace_buffer, zend_ssa *ssa, uint32_t parent_trace, uint32_t exit_num)
{
	...
	while (i < j) {
		...
		if ((ssa->vars[i].use_chain >= 0 /*|| ssa->vars[i].phi_use_chain*/)
		 ...
		 && zend_jit_var_supports_reg(op_array, ssa, i)) {
		 	RA_IVAL_START(i, 0);
			if (i < parent_vars_count
	 ...
	 || trace_buffer->stop == ZEND_JIT_TRACE_STOP_RECURSIVE_RET) {
		...
		while (phi) {
			...
			if (ssa->vars[phi->ssa_var].use_chain >= 0
			 ...
			 && zend_jit_var_supports_reg(op_array, ssa, phi->ssa_var)) {
		...
		if (p->op == ZEND_JIT_TRACE_VM) {
			...
			if (support_opline) {
				...
				if (ssa_op->op1_def >= 0
				 ...
				 && ssa->vars[ssa_op->op1_def].alias == NO_ALIAS
				 && zend_jit_var_supports_reg(op_array, ssa, ssa_op->op1_def)
				...
				if (ssa_op->op2_def >= 0
				 ...
				 && zend_jit_var_supports_reg(op_array, ssa, ssa_op->op2_def)
				...
				case ZEND_FRAMELESS_ICALL_3:
					...
					if (ssa_op->op1_use >= 0
					 ...
					 && !zend_ssa_is_no_val_use(opline, ssa_op, ssa_op->op1_use)) {
						if (support_opline) {
							if (opline->op1_type == IS_CV) {
								if (((opline-1)->opcode == ZEND_ASSIGN_DIM || (opline-1)->opcode == ZEND_ASSIGN_OBJ)
								 && (opline-1)->op1.var == opline->op1.var) {
									/* skip $o->prop = $o */
								} else {
									zend_jit_trace_use_var(idx, ssa_op->op1_use, ssa_op->op1_def, ssa_op->op1_use_chain,
			...
			for (i = 0; i < op_array->last_var; i++) {
				...
				if (ssa->vars[j].use_chain >= 0
				 ...
				 && zend_jit_var_supports_reg(op_array, ssa, j)) {
					RA_IVAL_START(j, idx);
			...
			while (phi) {
				...
				if (RA_HAS_REG(def)) {
					if (!RA_HAS_REG(use)) {
						...
						use = phi->sources[0];
						if (zend_jit_var_supports_reg(op_array, ssa, use)) {
						...
						&& (!ssa->vars[def].no_val
						 || (ssa->var_info[use].type & (MAY_BE_STRING|MAY_BE_ARRAY|MAY_BE_OBJECT)))) {
...
static void zend_jit_trace_cleanup_stack(zend_jit_ctx *jit, zend_jit_trace_stack *stack, const zend_op *opline, const zend_ssa_op *ssa_op, const zend_ssa *ssa, const zend_op **ssa_opcodes)
{
	if (ssa_op->op1_use >= 0
	 ...
	 && (opline->op1_type != IS_CV
	  || !(ssa->var_info[ssa_op->op1_use].type & (MAY_BE_REF|MAY_BE_STRING|MAY_BE_ARRAY|MAY_BE_OBJECT|MAY_BE_RESOURCE|MAY_BE_GUARD)))
	...
	if (ssa_op->op2_use >= 0
	 ...
	 && (opline->op2_type != IS_CV
	  || !(ssa->var_info[ssa_op->op2_use].type & (MAY_BE_REF|MAY_BE_STRING|MAY_BE_ARRAY|MAY_BE_OBJECT|MAY_BE_RESOURCE|MAY_BE_GUARD)))
	...
	if (ssa_op->result_use >= 0
	 ...
	 && (opline->result_type != IS_CV
	  || !(ssa->var_info[ssa_op->result_use].type & (MAY_BE_REF|MAY_BE_STRING|MAY_BE_ARRAY|MAY_BE_OBJECT|MAY_BE_RESOURCE|MAY_BE_GUARD)))
...
//					case ZEND_DIV: // TODO: check for division by zero ???
					...
					case ZEND_ASSIGN_DIM_OP:
						...
						if (!zend_jit_assign_dim_op(&ctx, opline,
								op1_info, op1_def_info, op1_addr, OP1_DEF_REG_ADDR(),
							...
							if (!left_frame) {
								for (j = 0 ; j < op_array->last_var; j++) {
									...
									if (info & (MAY_BE_STRING|MAY_BE_ARRAY|MAY_BE_OBJECT|MAY_BE_RESOURCE|MAY_BE_REF)) {
										...
										ref = STACK_REF(stack, j);
										if (ref && ref != IR_NULL) {
											jit_GC_PTR_DTOR(&ctx, ref, info, 1, NULL);
										} else if (!zend_jit_free_cv(&ctx, info, j)) {
											goto jit_failure;
										}
							...
							if (opline->opcode == ZEND_FETCH_THIS
							 && delayed_fetch_this) {
								...
								SET_STACK_REF_EX(stack, EX_VAR_TO_NUM(opline->result.var), ra[ssa_op->result_def].ref,
									...
									(ssa->var_info[ssa_op->result_def].avoid_refcounting ? ZREG_GC_ADDREF : 0));
		 ...
		 && opline->opcode != ZEND_INCLUDE_OR_EVAL) {
			for (i = 0; i < op_array->last_var + op_array->T; i++) {
				...
				if (ref && !(STACK_FLAGS(stack, i) & (ZREG_LOAD|ZREG_STORE))) {
					if (!zend_jit_store_ref(jit, 1 << type, i, ref, 1/*???STACK_MEM_TYPE(stack, i) != type*/)) {
						goto jit_failure;
					}
					SET_STACK_TYPE(stack, i, type, 1);
...
int ZEND_FASTCALL zend_jit_trace_exit(uint32_t exit_num, zend_jit_registers_buf *regs)
{
	...
	for (i = 0; i < stack_size; i++) {
		if (STACK_FLAGS(stack, i) == ZREG_CONST) {
			if (STACK_TYPE(stack, i) == IS_LONG) {
				...
				ZVAL_STR(EX_VAR_NUM(i), (zend_string*)t->constants[STACK_REF(stack, i)].p);
			} else if (STACK_TYPE(stack, i) == IS_ARRAY) {
				ZVAL_ARR(EX_VAR_NUM(i), (HashTable*)t->constants[STACK_REF(stack, i)].p);
			} else {
				ZEND_UNREACHABLE();
			}
			...
			if (type <= IS_DOUBLE) {
				...
				Z_TYPE_INFO_P(zv) = (GC_FLAGS(Z_STR_P(zv)) & GC_IMMUTABLE) ? IS_STRING : IS_STRING_EX;
			} else if (STACK_TYPE(stack, i) == IS_ARRAY) {
				zval *zv = EX_VAR_NUM(i);
				Z_TYPE_INFO_P(zv) = (GC_FLAGS(Z_ARR_P(zv)) & GC_IMMUTABLE) ? IS_ARRAY : IS_ARRAY_EX;
			} else if (STACK_TYPE(stack, i) == IS_OBJECT) {
				Z_TYPE_INFO_P(EX_VAR_NUM(i)) = IS_OBJECT_EX;
			} else {
				ZEND_UNREACHABLE();
			}
	...
	if (bad_exit) {
		...
		ZEND_ASSERT(opline);
		EX(opline) = op = opline - 1;
		if (op->opcode == ZEND_FETCH_DIM_R
				|| op->opcode == ZEND_FETCH_DIM_FUNC_ARG
				|| op->opcode == ZEND_FETCH_LIST_R) {
			if (op->op2_type == IS_CONST) {
				zv = RT_CONSTANT(op, op->op2);
			} else {
				zv = EX_VAR(op->op2.var);
			}
			if (Z_TYPE_P(zv) == IS_LONG) {
				zend_error(E_WARNING, "Undefined array key " ZEND_LONG_FMT, Z_LVAL_P(zv));
			} else if (Z_TYPE_P(zv) == IS_STRING) {
				zend_error(E_WARNING, "Undefined array key \"%s\"", Z_STRVAL_P(zv));
			} else {
				ZEND_UNREACHABLE();
			}
		} else if (op->opcode == ZEND_FETCH_OBJ_R
				|| op->opcode == ZEND_FETCH_OBJ_IS
				|| op->opcode == ZEND_FETCH_OBJ_FUNC_ARG) {
			...
			return 1;
		} else if (op->opcode == ZEND_FETCH_DIM_IS
				|| op->opcode == ZEND_FETCH_OBJ_IS) {
			/* pass */ // TODO: ???
//			bad_exit = 0;
		} else {
	...
	if (bad_exit || (t->exit_info[exit_num].flags & ZEND_JIT_EXIT_TO_VM)) {
